...
'SemEndRes Or SemTel Or SemProf deixar como Pendente
...
Partial Public Class frmCadastrarPerito
    ...
    Public SemEndRes As Boolean = True
    Public SemTel As Boolean = True
    Public SemProf As Boolean = True
        ...
        If Not Me.IsPostBack Then
            Try
                If GetUsuario.Login = "SANTO" Then
                    'BtnGravaGEDFotoDisco.Visible = True
                    'BtnGravaGEDCVDisco.Visible = True
    ...
    Protected Sub BtnGravar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles BtnGravar.Click
        ...
        If Not IsDate(txtDt_Nasc.Text) Then
            MsgErro("Data de nascimento inválida.")
            txtDt_Nasc.Focus()
            Exit Sub
        End If

        If Trim(txtDt_Nasc.Text) = "01/01/1900" Then
            MsgErro("Data de nascimento 01/01/1900 - inválida.")
            txtDt_Nasc.Focus()
            Exit Sub
        End If
        If Trim(txtDt_Nasc.Text) = "1/1/1900" Then
            MsgErro("Data de nascimento 1/1/1900 - inválida.")
            txtDt_Nasc.Focus()
            Exit Sub
        End If
        If Trim(txtDt_Nasc.Text) = "" Then
            MsgErro("Preencha a Data de nascimento. Campo Obrigatório.")
            txtDt_Nasc.Focus()
            Exit Sub
        End If
        ...
        End If
        'Pegando os telefones da Grid
        Dim telefone1 As String = ""
        Dim telefone2 As String = ""
        Dim telefone3 As String = ""
        Dim balTele As New BalTelefone(GetUsuario)
        Dim entTele As New EntTelefone
        Dim vetor(1) As String

        'Testar
        entTele = balTele.ConsultarTelefone(txtID_PF.Text, 1)
        If Not entTele Is Nothing Then
            telefone1 = Mid(entTele.Tel, 1, 49)
            entTele = balTele.ConsultarTelefone(txtID_PF.Text, 2)
            If Not ent Is Nothing Then
                telefone2 = Mid(entTele.Tel, 1, 49)
                entTele = balTele.ConsultarTelefone(txtID_PF.Text, 3)
                If Not entTele.Tel Is Nothing Then
                    telefone3 = Mid(entTele.Tel, 1, 49)
                Else
                    entTele = balTele.ConsultarTelefone(txtID_PF.Text, 4)
                    If Not entTele Is Nothing Then
                        telefone3 = Mid(entTele.Tel, 1, 49)
                    End If
                End If
            Else
                ...
                Else
                    ...
                    If Not entTele Is Nothing Then
                        telefone2 = Mid(entTele.Tel, 1, 49)
                    End If
                    entTele = balTele.ConsultarTelefone(txtID_PF.Text, 5)
                    If Not entTele Is Nothing Then
                        telefone3 = Mid(entTele.Tel, 1, 49)
                    End If
                End If
                ...
                Else
                    ...
                    Else
                        ...
                        If Not entTele Is Nothing Then
                            telefone2 = Mid(entTele.Tel, 1, 49)
                        End If
                        entTele = balTele.ConsultarTelefone(txtID_PF.Text, 6)
                        If Not entTele Is Nothing Then
                            telefone3 = Mid(entTele.Tel, 1, 49)
                        End If
                    End If
        ...
        ent.TEL1 = telefone2
        ent.TEL2 = telefone3
        'telefone2 = Mid(telefone2, 1, 49)
        'telefone3 = Mid(telefone3, 1, 49)
        '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        'Criticas da Procedure do Delfa
        'If length(sUF) <> 2 Then
        '    Return 'UF deve ser composta de 2 caracteres';
        'End If

        If Len(ent.Sigla_UF1) <> 2 Or Len(ent.Sigla_UF) <> 2 Then
            MsgErro("Gravação Rejeitada. UF deve ser composta de 2 caracteres")
            Exit Sub
        End If
        'If sNome Is null Then
        '    Return 'Nome não informado';
        'End If
        If txtNome.Text = "" Then
            MsgErro("Gravação Rejeitada. Nome não informado")
            lblValidaNome.Text = "Preencher Nome"
            Exit Sub
        End If
        'If length(sNome) > 250 Then
        '    Return 'Nome deve ter até 250 caracteres';
        'End If
        If Len(txtNome.Text) > 250 Then
            MsgErro("Gravação Rejeitada. Nome deve ter até 250 caracteres")
            lblValidaNome.Text = "Nome muito extenso"
            Exit Sub
        End If
        'If sCPF Is null Then
        '    Return 'CPF não informado';
        'End If
        If txtCPF.Text = "" Then
            MsgErro("Gravação Rejeitada. CPF não informado")
        ...
        If Len(txtCPF.Text) <> 11 Then
            MsgErro("Gravação Rejeitada.CPF deve ter 11 caracteres")
            lblValidaCPF.Text = "CPF inválido"
            Exit Sub
        End If
        'If sComplemento Is null Then
        '    Return 'Complemento não informado';
        'End If
        If txtID_PF.Text <> "" Then
            If ent.Nome_Logr = "" Then
                MsgErro("Gravação Rejeitada. Nome do Logradouro(Endereço) não informado")
                Exit Sub
            End If
            'If length(sComplemento) > 150 Then
            '    Return 'Complemento deve ter até 150 caracteres';
            'End If
            If Len(ent.Nome_Logr) > 150 Or Len(ent.Nome_Logr1) > 150 Then
                MsgErro("Gravação Rejeitada. Nome do Logradouro(Endereço) deve ter até 150 caracteres")
                Exit Sub
            End If
            'If sBairro Is null Then
            '    Return 'Bairro não informado';
            'End If
            If ent.Cod_Bairro1 = 0 And ent.Cod_Bairro = 0 Then
                MsgErro("Gravação Rejeitada. Bairro não informado")
                Exit Sub
            End If
            'If length(sBairro) > 80 Then
            '    Return 'Bairro deve ter até 80 caracteres';
            'End If
            If Len(ent.Descr_Bairro) > 80 Or Len(ent.Descr_Bairro1) > 80 Then
                MsgErro("Gravação Rejeitada. Cidade deve ter até 80 caracteres")
                Exit Sub
            End If
            'If sCidade Is null Then
            '    Return 'Cidade não informada';
            'End If
            If ent.Cod_Cidade1 = 0 And ent.Cod_Cidade = 0 Then
                MsgErro("Gravação Rejeitada. Cidade não informada")
                Exit Sub
            End If
            'If length(sCidade) > 80 Then
            '    Return 'Cidade deve ter até 80 caracteres';
            'End If
            If ent.Descr_Cidade1 = "" And ent.Descr_Cidade = "" Then
                MsgErro("Gravação Rejeitada. Cidade não informada")
                Exit Sub
            End If
            If Len(ent.Descr_Cidade1) > 80 Or Len(ent.Descr_Cidade) > 80 Then
                MsgErro("Gravação Rejeitada. Cidade deve ter até 80 caracteres")
                Exit Sub
            End If
            'If sCEP Is null Then
            '    Return 'CEP não informado';
            'End If
            If ent.CEP = "" And ent.CEP1 = "" Then
                MsgErro("Gravação Rejeitada. CEP não informado")
                Exit Sub
            End If
            'If length(sCEP) > 8 Then
            '    Return 'CEP deve ter até 8 caracteres';
            'End If
            If Len(ent.CEP) = 0 And Len(ent.CEP1) = 0 Then
                MsgErro("Gravação Rejeitada. CEP não informado")
                Exit Sub
            End If
        End If
        '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        'As validações dos telefones encontram-se nesta rotina junto ao inserirtelefonepessoafisica
        ''If length(sTel1) > 50 Then
        ''    Return 'Telefone 1 deve ter até 50 caracteres';
        ''End If
        ''If length(sTel2) > 50 Then
        ''    Return 'Telefone 2 deve ter até 50 caracteres';
        ''End If
        ''If length(sTel3) > 50 Then
        ''    Return 'Telefone 3 deve ter até 50 caracteres';
        ''End If
        ''If length(sFAX) > 40 Then
        ''    Return 'FAX deve ter até 40 caracteres';
        ''End If
        '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        'Teste
        ent.Cod_Tip_Sit = CInt(IIf(RdAtivo.Items(0).Selected, 1, 2))
        ent.OBS = "" 'txtObs.Text
        ent.EMAIL = txtEmail.Text '"SANTO@TJRJ.jus.BR"
        ...
        '---------------------------------------
        'Trocar lblProvisorio1 para lblProvisorio
        If lblInternet.Visible Then
            ent.SITUACAO_CADASTRO = "I"
            lblProvisorio.Visible = False
            TxtDiasPrazo.Visible = False
            TxtDiasPrazo.Enabled = False
        End If
        If lblProvisorio.Visible Then
            ent.SITUACAO_CADASTRO = "T"
            lblInternet.Visible = False
            'TxtDiasPrazo.Visible = true
            'lblProvisorio.Visible = True
            'TxtDiasPrazo.Visible = True
        ...
        Else
            Ret = Bal.VerPeritoPericias(txtCPF.Text, GetUsuario.Login)
            If Ret = 0 Then
                MsgErro("Inserção rejeitada. Existe Perito Cadastrado com este CPF")
            Else
                txtID_PF.Text = Ret.ToString
                ent.ID_PF = CInt(Convert.ToInt64(txtID_PF.Text))
                'logger.Debug("Bal.Gravar(ent, InserirConta)")
                Bal.Gravar(ent, InserirConta)
                txtCod_Perito.Text = ent.Cod_Perito.ToString
            End If
        ...
        End If
        '''''''''''''''''''''''''''
        If RdSit.Items(0).Selected = True Then 'Somente grava os dados bancários quando a situação está ok.
            If txtNum_Conta_Corrente.Text = "" And TxtNum_Agencia.Text = "" And CboBanco.SelectedIndex = 0 And txtNome_Agencia.Text = "" Then
                ...
                HabilitaBancarios(True)
            Else
                'BtnDadosBancarios.Enabled = False
                HabilitaBancarios(False)
            End If
        Else
            'BtnDadosBancarios.Enabled = False
            HabilitaBancarios(False)
        ...
        'No Oracle -> Function Grava_TmpForn(sNome varchar2, sComplemento varchar2, sBairro varchar2, sCidade varchar2, sCEP varchar2, sTel1 varchar2,
        '                                    sTel2 varchar2, sTel3 varchar2, sFAX varchar2, sCPF varchar2, sUF varchar2) return varchar2 is
        'Esta tabela será utilizada pelas procedures do Delfa para gravação dos dados dos fornecedores

        'CodTipSituacao = sd.ExecutaFunc("verifica_situacao_perito", 10, entBal1.ID_PF)

        If RdSit.Items(0).Selected _
           And Trim(lblExcluido.Text) = "" Then
            'And RdAtivo.Items(0).Selected Then 'somente grava fornecedore se status ok --------e ativo
            'p.situacao_cadastro = 'O' and
            '(p.data_exclusao = '01/01/1900' or p.data_exclusao is null) then
            ...
            End If
        Else
            MsgErro("Gravação do Fornecedor Rejeitada. Perito Inativo ou possui Pendência")
        End If
        ...
        End If
        '------------------------------------------------------
        'Desativar o Perito no Cadastro(Antigo) do DCP
        'Verificar se existe perito ativo 
        Dim BalPerito_Comarca As New BalPerito_Comarca(GetUsuarioExt)
        'Dim dsDesPer As DataSet
        'Session.Remove("DsDesPer")

        'Método comenta pois estava apresentando erro.
        'BalPerito_Comarca.Desativar_Per_DCP(Trim(UCase(txtNome.Text)))
        ...
        '    'Procedimento provisório até a conclusão do recadastramento na DIPEJ
        '    If UPPER(txtNome.Text) = "JOSÉ CARLOS SILVEIRA BOUSQUET" Then
        '        'Cod_Perit =609
        '        MsgErro("Existem dois ou mais registros com este nome. Desejas desativar no DCP o perito com RG : Núm. Reg = 03374809?")
        '        'Cod_Perit =889 
        '        MsgErro("Existem dois registros com este nome. Desejas desativar no DCP o perito com RG : Núm. Reg = 102672?")
        '    ElseIf UPPER(txtNome.Text) = "TARCISO GOUVEIA DA SILVA" Then
        '        'Cod_Perit = 972 
        '        MsgErro("Existem dois registros com este nome. Desejas desativar no DCP o perito com RG : Núm. Reg = 226092?")
        '        'Cod_Perit = 837
        '        MsgErro("Existem dois registros com este nome. Desejas desativar no DCP o perito com RG : Núm. Reg = 140521?")
        '        'ElseIf UPPER(Nome) = "RENATO DA SILVA CHAGAS" Then
        '        'ERRO DE DIGITAÇÃO DO CÓDIGO - DESATIVAR OS DOIS
        '        'Cod_Perit = 69
        '        '    MsgErro("Existem dois registros com este nome. Desejas desativar no DCP o perito com RG : Núm. Reg = 14052?"
        '        'Cod_Perit = 203
        '        '    MsgErro("Existem dois registros com este nome. Desejas desativar no DCP o perito com RG : Núm. Reg = 140521?"                         
        '    Else
        '        'Desativar (Sit_Oper = 1 em PeritoDCP no servidor do DCP) pelo Nome
        '    End If

        '    'Me.ClientScript.RegisterClientScriptBlock(Me.GetType(), "NovaJanela", "window.open('frmDesativarPerDCP.aspx', '_blank', 'height=550,width=500, Top=50,left=120');", True)
        'End If
        ''''''''''''''''''''''''''''''''''''''''''''''''
    End Sub
    ...
    Protected Sub BtnExcluir_Click(ByVal sender As Object, ByVal e As EventArgs) Handles BtnExcluir.Click
        ...
        If Session("CodTipFunc").ToString <> "4" Then
    ...
    Private Sub PreencherDadosPerito()
        ...
        If Not EntPer Is Nothing Then
            ...
            End If
            Session("codTipFunc") = EntPer.CodTipFunc
            ...
            If EntPer.Dt_Nasc.ToString Is Nothing Or EntPer.Dt_Nasc.ToString = "01/01/0001 00:00:00" Then
                txtDt_Nasc.Text = ""
            Else
                ...
                Else
                    ...
                    If Trim(txtDt_Nasc.Text) = "01/01/1900" Or Trim(txtDt_Nasc.Text) = "1/1/1900" Or Trim(txtDt_Nasc.Text) = "1/1/0001" Or Trim(txtDt_Nasc.Text) = "01/01/0001" Then
                        txtDt_Nasc.Text = ""
                    End If
                End If
            End If
            ...
            Else
                ...
                HabilitaBancarios(False)
            End If
            If EntPer.SITUACAO_CADASTRO = "I" Then
                lblInternet.Visible = True
            Else
                lblInternet.Visible = False
            End If
            If EntPer.SITUACAO_CADASTRO = "T" Then
                lblProvisorio.Visible = True
                'TxtDiasPrazo.Visible = True
                'TxtDiasPrazo.Enabled = True
            Else
    ...
    Protected Sub txtNome_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles txtNome.TextChanged
        ...
        If txtNome.Text <> "" Then
            If txtCPF.Text = "" Then 'Desta forma permite corrigir o nome do Perito.'
    ...
    Protected Sub RdSit_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles RdSit.SelectedIndexChanged
        ...
        Dim balProf As New BALEspecialidadePerito(GetUsuario)
        Dim ds As DataSet
        ...
        If RdSit.Items(0).Selected And (Not chkDOCNECCV.Checked Or _
            ...
            Exit Sub
        End If
        If TxtCEP.Text = "" Or txtNum_Logr.Text = "" Or txtNome_Logr.Text = "" Or CboBairro.Text = "" Or CboTip_Logr.Text = "" Or CboCidade.Text = "" Or CboTip_Logr.Text = "" Or CboUF.Text = "" Then
            SemEndRes = True
        Else
            SemEndRes = False
        End If
        If SemEndRes Then
            RdSit.Items(1).Selected = True
            MsgErro("Não existe Endereço Residencial cadastrado para este perito.")
            Exit Sub
        End If
        ...
        If Not ds Is Nothing Then
            'If Not GrdTel.DataSource Is Nothing Then
            SemTel = False
        Else
            SemTel = True
        End If
        ...
        If SemTel Then
            RdSit.Items(1).Selected = True
            MsgErro("Não existe Telefone cadastrado para este perito.")
            Exit Sub
        End If

        'GrdProfissao.DataSource = balProf.ListarEspecialidadesPerito(txtID_PF.Text.ToString)
        ds = balProf.ListarEspecialidadesPerito(txtID_PF.Text.ToString)

        'If Not GrdProfissao.DataSource Is Nothing Then
        If Not ds Is Nothing Then
            SemProf = False
        Else
            SemProf = True
        ...
        If SemProf Then
            RdSit.Items(1).Selected = True
            MsgErro("Não existe Profissão cadastrado para este perito.")
            Exit Sub
        End If
    ...
    Protected Sub CboOrgao_Per_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles CboOrgao_Per.SelectedIndexChanged
        ...
        If Not CboOrgao_Per.SelectedIndex = 0 Then
            PreencherSiglaOrgaoProfissional(CInt(CboOrgao_Per.SelectedValue))
        Else
    ...
    Private Sub HabilitaBotoes(ByVal vbol As Boolean)
        ...
        '[CPF NÃO CADASTRADO COMO PERITO]
        ...
        BtnGravaCurriculum.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And lblExcluido.Text = "", True, False))
        BtnExibirCurriculum.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And lblExcluido.Text = "", True, False))
        btnGravarTel.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))
        btnGravarProfissao.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))

        'BtnDadosBancarios.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "", True, False))
        HabilitaBancarios(CBool(IIf(vbol And txtID_PF.Text <> "", True, False)))
        BtnNur1.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))
        BtnNur2.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))
        BtnNur3.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))
        BtnNur4.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))
        BtnNur5.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))
        BtnNur6.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))
        BtnNur7.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))
        BtnNur8.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))
        BtnNur9.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))
        BtnNur10.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))
        BtnNur11.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))
        BtnNur12.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))
        BtnNur13.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))

        BtnTel.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))
        BtnInf.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))
        BtnEndCom.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))
        BtnEndRes.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))
        BtnChklist.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))
        BtnProfissao.Enabled = CBool(IIf(vbol And txtID_PF.Text <> "" And (lblExcluido.Text = "" Or lblExcluido.Text = "[CPF NÃO CADASTRADO COMO PERITO]"), True, False))
...
#Region "Manipulação do Telefone"
    ...
    Protected Sub btnGrdAlterar_Command(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs)
        ...
        End If
        btnGravarTel.Text = "Alterar Telefone"
    End Sub
    ...
    Protected Sub btnGravarTel_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnGravarTel.Click
        ...
        'Inserir Critica do Delfa para telefones
        ''If length(sTel1) > 50 Then
        ''    Return 'Telefone 1 deve ter até 50 caracteres';
        ''End If
        If Len(ent.Tel) > 50 Then
            MsgErro("Gravação Rejeitada. O telefone deve ter até 50 caracteres")
            Exit Sub
        End If
        'Fax será sempre vazio - vide procedure
        ''If length(sFAX) > 40 Then
        ''    Return 'FAX deve ter até 40 caracteres';
        ''End If

        bValida = bal.Validar(ent, CLng(txtID_PF.Text))

        If btnGravarTel.Text = "Alterar Telefone" Then
            bal.AlterarPessoaFisicaTelefone(ent, txtID_PF.Text)
        ElseIf (Not bValida) And btnGravarTel.Text = "Inserir Telefone" Then
            bal.InserirPessoaFisicaTelefone(ent, CLng(txtID_PF.Text))
        Else
            sMsg = "Telefone já existe."
        End If
        ...
        CboTip_Tel.SelectedIndex = 0
        ListaTelefones()
        btnGravarTel.Text = "Inserir Telefone"
    End Sub
...
#Region "Manipulação de informações da profissão do perito"
    ...
    Protected Sub btnGrdAlterarProf_Command(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs)
        ...
        If Not CboProfissao.Items.FindByValue(vetor(1)) Is Nothing Then
            CboProfissao.SelectedValue = CStr(vetor(1))
        Else
            CboProfissao.SelectedIndex = 0
        End If
        ...
        If Not CboEspecialidade.Items.FindByValue(CStr(vetor(2))) Is Nothing Then
            CboEspecialidade.SelectedValue = CStr(vetor(2))
        Else
            CboEspecialidade.SelectedIndex = 0
        End If
        ...
        If Not CboOrgao_Per.Items.FindByValue(CStr(vetor(6))) Is Nothing Then
            CboOrgao_Per.SelectedValue = CStr(vetor(6))
        Else
            CboOrgao_Per.SelectedIndex = 0
        End If
        btnGravarProfissao.Text = "Alterar Profissão"
    End Sub
    ...
    Protected Sub btnExcluirProf_Command(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs)
        ...
        bal.Excluir(CLng(vetor(0)), CInt(vetor(1)), CInt(vetor(2)))
    ...
    Protected Sub btnGravarProfissao_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnGravarProfissao.Click
        ...
        bValida = bal.Validar(ent)

        If btnGravarProfissao.Text = "Alterar Profissão" Then
            bal.Alterar(ent)
        ElseIf (Not bValida) And btnGravarProfissao.Text = "Inserir Profissão" Then
            sMsg = bal.Inserir(ent)
            If sMsg <> "" Then
                bal.Alterar(ent)
                sMsg = ""
            End If
        Else
            sMsg = "Profissão não foi gravada."
        End If
        ...
        ListarProfissaoPerito()
        btnGravarProfissao.Text = "Inserir Profissão"
    End Sub
#End Region
    ...
    Private Function DBNull() As DataSet
        Throw New NotImplementedException
    End Function

    Private Function UPPER(ByVal p1 As Object) As String
        Throw New NotImplementedException
    End Function

    Protected Sub BtnGravaGEDFotoDisco_Click(ByVal sender As Object, ByVal e As EventArgs) Handles BtnGravaGEDFotoDisco.Click
        Dim Bal As New BALPERITO(GetUsuario)
        ...
        Total = Bal.TotalIDTodosPeritos()
        DSet = Bal.ExibirIDTodosPeritos()
        ValorIni = CInt(txtNumIni.Text) - 1
        'Gravar em Disco (Origem Oracle)
        'For ii = ValorIni To Total - 1
        '    txtID_PF.Text = DSet.Tables(0).Rows(ii).Item(0).ToString
        '    GravarFotoGED(txtID_PF.Text)
        '    'Contador = Contador + 1
        'Next
        'Gravar no Oracle (origem Disco)
        For ii = ValorIni To Total - 1
            txtID_PF.Text = DSet.Tables(0).Rows(ii).Item(0).ToString
            GravarFotoGEDOracle(txtID_PF.Text)
            'Contador = Contador + 1
        Next

    End Sub

    Private Sub GravarFotoGED(ByVal New_ID_PF As String)
        ...
        If f_ID_PF <> "" Then
            ...
            If IDDoc <> "" Then
                ...
                'End If
                'Else
                NomeArqFoto = "E:\GED\Foto\Foto" + f_ID_PF + ".jpg"
                Obj.Inicializa(GetUsuario.Login, "", GetUsuario.NomeMaquina, "PERICIAS", GetUsuario.UsuarioSO, GetUsuario.CodOrg.ToString, DGTECGEDAR.TipoServidorIndexacao.Homologacao2, DGTECGEDAR.TipoServidorWebService.Automatico, False)
                ''''''''''strImagem = Obj.Recupera(IDDoc, Server.MapPath("Imagens/FotoPerito.jpg"))
                strArquivo = Obj.Recupera(IDDoc, NomeArqFoto)
                Obj.Finaliza()
                'End If
                'Else
                'MsgErro("O Perito não possui foto!")
                'Gravar em Arquivo no Disco Local
                'NomeArqFoto = "E:\GED\Foto\Foto" + f_ID_PF + ".bmp"
                ''''Dim objBmp As New Bitmap(strImagem)
                ''''objBmp.Save(NomeArqFoto.ToString)
                'End If
            End If
        End If
    ...
    Private Sub GravarFotoGEDOracle(ByVal New_ID_PF As String)
        ...
        If f_ID_PF <> "" Then
            ...
            IDDoc = Obj.Armazena(NomeArqFoto)
            Obj.Finaliza()
    ...
    Protected Sub BtnGravaGEDCVDisco_Click1(ByVal sender As Object, ByVal e As EventArgs) Handles BtnGravaGEDCVDisco.Click
        Dim Bal As New BALPERITO(GetUsuario)
        Dim Contador As Integer = 0
        ...
        'For ii = ValorIni To Total - 1
        'txtID_PF.Text = DSet.Tables(0).Rows(ii).Item(0).ToString
        'GravarCVGED(txtID_PF.Text)
        'Contador = Contador + 1
        'Next
        'Gravar em Oracle (Origem Disco)
        For ii = ValorIni To Total - 1
            txtID_PF.Text = DSet.Tables(0).Rows(ii).Item(0).ToString
            GravarCVGEDOracle(txtID_PF.Text)
            'Contador = Contador + 1
        Next
    End Sub
    Private Sub GravarCVGED(ByVal New_ID_PF As String)
        ...
        If IDDoc <> "" Then
            ...
            'Else
            'Gravar em Arquivo no Disco Local
            NomeArqCV = "E:\GED\CV\CV" + f_ID_PF + ".pdf"
            Obj.Inicializa(GetUsuario.Login, "", GetUsuario.NomeMaquina, "PERICIAS", GetUsuario.UsuarioSO, GetUsuario.CodOrg.ToString, DGTECGEDAR.TipoServidorIndexacao.Homologacao2, DGTECGEDAR.TipoServidorWebService.Automatico, False)
            strArquivo = Obj.Recupera(IDDoc, NomeArqCV)
            Obj.Finaliza()
            'System.IO.File.Copy(x, NomeArqCV)
            'End If
        End If

    End Sub
    Private Sub GravarCVGEDOracle(ByVal New_ID_PF As String)
        Dim Bal As New BALPERITO(GetUsuario)
        Dim Ent As New EntPERITO
        Dim TipoServIndexGED As New DGTECGEDARDOTNET.DGTECGEDAR.TipoServidorIndexacao
        ...
        If f_ID_PF <> "" Then
            ...
            End If
            Obj.Inicializa(GetUsuario.Login, "", GetUsuario.NomeMaquina, "PERICIAS", GetUsuario.UsuarioSO, GetUsuario.CodOrg.ToString, TipoServIndexGED, DGTECGEDAR.TipoServidorWebService.Automatico, False)
            IDDoc = Obj.Armazena(NomeArqCV)
            Obj.Finaliza()
            'GRAVAR ID DO GED na Tabela Perito
            Bal.GravarCurriculum(f_ID_PF, IDDoc)
        End If

    End Sub

    Private Sub HabilitaBancarios(ByVal Habilita As Boolean)

        BtnDadosBancarios.Enabled = Habilita
        TxtNum_Agencia.Enabled =
        txtNome_Agencia.Enabled = Habilita
        txtNum_Conta_Corrente.Enabled = Habilita
        'txtFalta_Entregar.Enabled = Habilita
        CboBanco.Enabled = Habilita
