...
Partial Public Class frmPreCadastroPeritoExt
    ...
    Protected Sub BtnGravar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles BtnGravar.Click
        ...
        'inserir o método para carregar no GED e na tabela de arquivos com referencia ao perito
    ...
    Protected Sub btnInserirArquivo_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnInserirArquivo.Click

        Dim ListaArquivos As ArrayList = New ArrayList

        If (FileUploadArquivos.HasFile) Then

            If FileUploadArquivos.PostedFile.ContentLength < 102400 Then

                If Not (Session("ListaArquivos") Is Nothing) Then

                    ListaArquivos = Session("ListaArquivos")

                End If

                For Each arquivo As ClienteWebPadrao.DGTECFileUpload In ListaArquivos

                    If arquivo.FileName = FileUploadArquivos.FileName Then

                        MsgErro("Arquivo já carregado.")
                        Exit Sub

                    End If
                Next

                ListaArquivos.Add(FileUploadArquivos)
                carregaGridArquivos(ListaArquivos)
    ...
    Public Sub carregaGridArquivos(ListaArquivos As ArrayList)

        Dim dr As DataRow
        Dim dtArquivo As New DataTable
        Dim Arquivo As New ClienteWebPadrao.DGTECFileUpload

        dtArquivo.Columns.Add("nome_arquivo")

        For Each Arquivo In ListaArquivos

            dr = dtArquivo.NewRow()
            dr("nome_arquivo") = System.IO.Path.GetFileName(Arquivo.FileName)
            dtArquivo.Rows.Add(dr)
            dtArquivo.AcceptChanges()

        Next

        Session.Add("ListaArquivos", ListaArquivos)

        GrdArquivo.DataSource = dtArquivo
        GrdArquivo.DataBind()

        'Dim theFileName As String
        'FileUploadArquivos.SaveAs("c:\a.txt")
        'FileUploadArquivos.SaveAs("c:\ArquivosPericias\a.txt")
        'upload.SaveAs(theFileName)
        'theFileName = System.IO.Path.Combine(Server.MapPath("~/Uploads"), Arquivo.FileName)

        'If System.IO.File.Exists(theFileName) Then
...
#Region "Manipulação dos dados de Profissão"
    ...
    Protected Sub btnExcluirArquivo_Command(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs)
        'logger.Debug("btnExcluirArquivo_Command ...")
        Dim NomeArquivo As String
        Dim novaListaArquivos As ArrayList
        Dim ListaArquivos As ArrayList

        NomeArquivo = e.CommandArgument.ToString()
        ListaArquivos = Session("ListaArquivos")

        novaListaArquivos = ListaArquivos.Clone

        For Each arquivo As ClienteWebPadrao.DGTECFileUpload In ListaArquivos

            If arquivo.FileName = NomeArquivo Then

                novaListaArquivos.Remove(arquivo)

            End If

        Next

        Session("ListaArquivos") = novaListaArquivos

        carregaGridArquivos(novaListaArquivos)
